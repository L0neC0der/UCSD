...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		List<Integer> wapas =new ArrayList<Integer>();
		for(int i=0;i<numVertices;i++){
			wapas.add(getNeighbors(i).size()+getInNeighbors(i).size());
		}
		Comparator cmp=Collections.reverseOrder();
		Collections.sort(wapas,cmp);
		return wapas;
	}
	...
	public static void main (String[] args) {
		//GraphLoader.createIntersectionsFile("data/maps/jojoe.map", "data/intersections/jojoe.intersections");
		...
		GraphLoader.loadRoadMap("data/maps/jojoe.map", graphFromFile);
