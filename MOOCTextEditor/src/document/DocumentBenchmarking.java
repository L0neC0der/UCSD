...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
				...
				numToCheck += increment)
		{
			...
			//Clocking time for Basic Documents
			starttime=System.nanoTime();
			for(int i=0;i<trials;i++){
			BasicDocument BD=new BasicDocument(text);
			score=BD.getFleschScore(); 
			}
			stoptime=System.nanoTime();
			time=((double)(stoptime-starttime))/(1000000000*trials);
			System.out.print(time+"\t");
			//Clocking time for Efficient Documents
			starttime=System.nanoTime();
			for(int i=0;i<trials;i++){
				EfficientDocument ED=new EfficientDocument(text);
				score=ED.getFleschScore();
			}
			stoptime=System.nanoTime();
