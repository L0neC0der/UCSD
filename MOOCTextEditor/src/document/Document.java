...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i=0; i<wo.length;i++){
			if(i==0 && match(wo[i])){
			...
			else if(i<(wo.length-1)){
				if(match(wo[i])&&!match(wo[i-1])){
				      check=false;
				      count++;
				      }
			      }
				...
				if(wo[i]=='e'&& check==true){
				count=1;
			                }
				else if(match(wo[i])&&wo[i]!='e'&&!match(wo[i-1])){
					count++;
				  }
			    }
			}
	...
	//function to match the vowels to the character
	private boolean match(char a){
